...
namespace Demo
{
	public partial class DemoForm : Form
	{
		...
		protected RenderTechniqueFluid		m_RenderTechniqueFluid = null;	// The advanced render technique for mesh fluid
...
//			BuildCaseTable();	// This table has holes because of missing cases :(
			...
			m_RenderTechniqueFluid = ToDispose( new RenderTechniqueFluid( m_Device, "Voxel Terrain Render Technique" ) );
			m_Renderer.Renderer.FindPipeline( Pipeline.TYPE.MAIN_RENDERING ).InsertTechnique( 0, m_RenderTechniqueFluid );	// Insert our technique at the beginning
			m_RenderTechniqueFluid.Camera = m_Renderer.Camera;
...
//			SceneTextureProvider	TextureProvider = ToDispose( new SceneTextureProvider( m_Device, "TextureProvider from disk", new System.IO.DirectoryInfo( "./Meshes/Masha" ) ) );
		...
		public void	RunMessageLoop()
		{
			...
			CamManip.InitializeCamera( new Vector3( 7.0f, 2.0f, 2.0f ), new Vector3( 2.0f, 2.0f, 2.0f ), Vector3.UnitY );
			...
			SharpDX.Windows.RenderLoop.Run( this, () =>
			{
				...
				using ( Graphics G = Graphics.FromHwnd( panelOutput.Handle ) )
					G.DrawString( "Move the mouse around the blue rectangle and press space to draw...", Font, Brushes.Black, 0, 0 );

			} );
		}

		// Use this message loop to test the basic 2D water effect...
		float[,]		B0 = new float[256,256];
		float[,]		B1 = new float[256,256];
		float[,]		B2 = new float[256,256];
		public unsafe void	RunMessageLoop2()
		{
			System.Drawing.Bitmap	B = new System.Drawing.Bitmap( 256, 256, System.Drawing.Imaging.PixelFormat.Format24bppRgb );
			...
			SharpDX.Windows.RenderLoop.Run( this, () =>
			{
				...
				System.Drawing.Imaging.BitmapData	Lock = B.LockBits( new Rectangle( 0, 0, 256, 256 ), System.Drawing.Imaging.ImageLockMode.WriteOnly, System.Drawing.Imaging.PixelFormat.Format24bppRgb );

				for ( int Y=0; Y < 256; Y++ )
				{
					byte*	pScan = (byte*) Lock.Scan0.ToPointer() + Lock.Stride * Y;
...
//						float	V = ((B1[X0,Y] + B1[X1,Y] + B1[X,Y0] + B1[X,Y1]) >> 1) - B0[X,Y];
						float	V = ((B1[X0,Y] + B1[X0,Y0] + B1[X1,Y] + B1[X1,Y0] + B1[X0,Y1] + B1[X1,Y1] + B1[X,Y0] + B1[X,Y1]) * 0.25f) - B0[X,Y];
						V *= 0.95f;
						B2[X,Y] = V;

						byte	C = (byte) Math.Min( 255, Math.Max( 0, V * 255 ) );
						*pScan++ = C;
						*pScan++ = C;
						*pScan++ = C;
					}
		...
		#region Triangle Cases for marching cubes
...
//					Writer.Write( "uint3( {0]{1:G2}, {2]{3:G2}, {4]{5:G2} ), ", T.m_EdgeIndex0 < 10 ? " " : "", T.m_EdgeIndex0, T.m_EdgeIndex1 < 10 ? " " : "", T.m_EdgeIndex1, T.m_EdgeIndex2 < 10 ? " " : "", T.m_EdgeIndex2 );
		...
		private void panelOutput_PreviewKeyDown( object sender, PreviewKeyDownEventArgs e )
		{
			if ( e.KeyCode == Keys.Space )
				m_RenderTechniqueFluid.Draw = true;
		}

		private void panelOutput_MouseMove( object sender, MouseEventArgs e )
		{
			m_RenderTechniqueFluid.MousePosition = e.Location;


			if ( e.X >= 256 || e.Y >= 256 )
				return;

			double	fExpFactor = Math.Log( 0.1 ) / 16.0;

			for ( int X=e.X-4; X <= e.X+4; X++ )
				for ( int Y=e.Y-4; Y <= e.Y+4; Y++ )
				{
					float	fSqDistance = (X-e.X)*(X-e.X)+(Y-e.Y)*(Y-e.Y);
					float	fValue = 1.0f * (float) Math.Exp( fExpFactor * fSqDistance );

					B1[X&0xFF,Y&0xFF] = fValue;
				}
		}
