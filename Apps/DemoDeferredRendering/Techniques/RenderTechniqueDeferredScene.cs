...
// 				Vertices[VertexIndex].BiTangent = (Vector3) _VertexFieldProvider.GetField( VertexIndex, VertexFieldIndex );
			...
			if ( _IndicesCount < 65536 )
			{
				UInt16[]	Indices = new UInt16[_IndicesCount];
				...
				Result = new Primitive<VS_P3N3G3T2,UInt16>( m_Device, _Name, PrimitiveTopology.TriangleList, Vertices, Indices );
			}
			else
			{
				// Query indices
				...
				// Build the actual primitive
				Result = new Primitive<VS_P3N3G3T2,int>( m_Device, _Name, PrimitiveTopology.TriangleList, Vertices, Indices );
			}
		...
		{
			if ( _Primitive is Primitive<VS_P3N3G3T2,UInt16> )
			{
				Primitive<VS_P3N3G3T2,UInt16>	P = _Primitive as Primitive<VS_P3N3G3T2,UInt16>;
			...
			else if ( _Primitive is Primitive<VS_P3N3G3T2,int> )
			{
				Primitive<VS_P3N3G3T2,int>	P = _Primitive as Primitive<VS_P3N3G3T2,int>;
				...
				throw new NException( this, "Provided primitive is not recognized by that technique !" );
		}
