...
namespace FBXImporter
{
	...
	public:		// METHODS
		...
		Layer( NodeMesh^ _Owner, KFbxLayer* _pLayer ) : m_Owner( _Owner )
		{
			// Build layer elements
			m_Elements = gcnew List<LayerElement^>();
			for ( int LayerElementType=0; LayerElementType < KFbxLayerElement::eLAST_ELEMENT_TYPE; LayerElementType++ )
			{
				KFbxLayerElement::ELayerElementType	ElementType = (KFbxLayerElement::ELayerElementType) LayerElementType;
				KFbxLayerElement*	pElement = _pLayer->GetLayerElementOfType( ElementType );
				if ( pElement == NULL )
