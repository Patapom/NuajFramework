...
Node::Node( Scene^ _ParentScene, Node^ _Parent, KFbxNode* _pNode ) : BaseObject( _ParentScene, _pNode ), m_Parent( _Parent )
{
//	m_bVisible = _pNode->GetVisibility();
	...
	WMath::Point^	Position = Helpers::ToPoint( _pNode->LclTranslation.Get() );
	WMath::Vector^	Rotation = (float) Math::PI / 180.0f * Helpers::ToVector( _pNode->LclRotation.Get() );
	WMath::Vector^	Scale    = Helpers::ToVector( _pNode->LclScaling.Get() );
...
Material^	Node::ResolveMaterial( KFbxSurfaceMaterial* _pMaterial )
{
