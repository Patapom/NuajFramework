...
namespace FMOD
{
    ...
    */
    internal class MEMBITS
    {
        public const uint OTHER = (1 << (int)MEMTYPE.OTHER);               /* Memory not accounted for by other types */
        public const uint STRING = (1 << (int)MEMTYPE.STRING);              /* String data */

        public const uint SYSTEM = (1 << (int)MEMTYPE.SYSTEM);              /* System object and various internals */
        public const uint RECORDBUFFER = (1 << (int)MEMTYPE.RECORDBUFFER);        /* Buffer used to store recorded data from microphone */
        public const uint PLUGINS = (1 << (int)MEMTYPE.PLUGINS);             /* Plugin objects and internals */
        public const uint OUTPUTMODULE = (1 << (int)MEMTYPE.OUTPUTMODULE);        /* Output module object and internals */
        public const uint SPEAKERLEVELSPOOL = (1 << (int)MEMTYPE.SPEAKERLEVELSPOOL);   /* Pool of speaker levels structs */
        public const uint CHANNELI = (1 << (int)MEMTYPE.CHANNELI);            /* Channel object */
        public const uint CHANNELPOOL = (1 << (int)MEMTYPE.CHANNELPOOL);         /* Channel pool data */
        public const uint CHANNELREAL = (1 << (int)MEMTYPE.CHANNELREAL);         /* Real channel objects */
        public const uint CHANNELGROUP = (1 << (int)MEMTYPE.CHANNELGROUP);        /* ChannelGroup objects and internals */
        public const uint SOUNDI = (1 << (int)MEMTYPE.SOUNDI);              /* Sound objects and internals */
        public const uint SOUNDGROUP = (1 << (int)MEMTYPE.SOUNDGROUP);          /* SoundGroup objects and internals */
        public const uint SAMPLE = (1 << (int)MEMTYPE.SAMPLE);              /* Sample objects */
        public const uint DSPCONNECTION = (1 << (int)MEMTYPE.DSPCONNECTION);       /* DSPConnection objects and internals */
        public const uint DSPCONNECTIONPOOL = (1 << (int)MEMTYPE.DSPCONNECTIONPOOL);   /* DSPConnection pool data */
        public const uint DSPI = (1 << (int)MEMTYPE.DSPI);                /* DSP implementation objects */
        public const uint DSPCODEC = (1 << (int)MEMTYPE.DSPCODEC);            /* Realtime file format decoding DSP objects */
        public const uint DSPUNIT = (1 << (int)MEMTYPE.DSPUNIT);             /* DSP unit objects */
        public const uint REVERBI = (1 << (int)MEMTYPE.REVERBI);             /* Reverb implementation objects */
        public const uint REVERBCHANNELPROPS = (1 << (int)MEMTYPE.REVERBCHANNELPROPS);  /* Reverb channel properties structs */
        public const uint GEOMETRY = (1 << (int)MEMTYPE.GEOMETRY);            /* Geometry objects and internals */
