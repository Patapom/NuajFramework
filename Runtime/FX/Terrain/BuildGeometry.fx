...
cbuffer cbLOD
{
	float	TextureSize = 1 + 65 + 1;
	float2	InvTextureSize = float2( 1.0 / (1 + 65 + 1), 0 );
...
GS_IN VS( VS_IN _In )
{
	...
	float3	VoxelCoordinates = CellCoordinates * InvVoxelCellsCount.x;
	...
	// Compute the size of a cell in density texture space (as the amount of cells in a block may not be equal to the amount of density cells in the texture)
	float2	Delta = float2( InvTextureSize.x * (TextureSize-3) * InvVoxelCellsCount, 0 );

	GS_IN	Out;
	Out.Field0123.x = DensityVolumeTexture.SampleLevel( NearestClamp, UVW + Delta.yyx, 0).x;
	Out.Field0123.y = DensityVolumeTexture.SampleLevel( NearestClamp, UVW + Delta.yxx, 0).x;
	Out.Field0123.z = DensityVolumeTexture.SampleLevel( NearestClamp, UVW + Delta.xxx, 0).x;
	Out.Field0123.w = DensityVolumeTexture.SampleLevel( NearestClamp, UVW + Delta.xyx, 0).x;
	Out.Field4567.x = DensityVolumeTexture.SampleLevel( NearestClamp, UVW + Delta.yyy, 0).x;
	Out.Field4567.y = DensityVolumeTexture.SampleLevel( NearestClamp, UVW + Delta.yxy, 0).x;
	Out.Field4567.z = DensityVolumeTexture.SampleLevel( NearestClamp, UVW + Delta.xxy, 0).x;
	Out.Field4567.w = DensityVolumeTexture.SampleLevel( NearestClamp, UVW + Delta.xyy, 0).x;
...
float3	ComputeVertexNormal( float3 _UVW )
{
	...
	float3	Gradient;
	Gradient.x =  DensityVolumeTexture.SampleLevel( LinearClamp, UVW + Delta.xyy, 0).x
				- DensityVolumeTexture.SampleLevel( LinearClamp, UVW - Delta.xyy, 0).x;
	Gradient.y =  DensityVolumeTexture.SampleLevel( LinearClamp, UVW + Delta.yxy, 0).x
				- DensityVolumeTexture.SampleLevel( LinearClamp, UVW - Delta.yxy, 0).x;
	Gradient.z =  DensityVolumeTexture.SampleLevel( LinearClamp, UVW + Delta.yyx, 0).x
				- DensityVolumeTexture.SampleLevel( LinearClamp, UVW - Delta.yyx, 0).x;
...
[maxvertexcount(15)]
void GS( point GS_IN _In[1], inout TriangleStream<GS_OUT> _Stream )
{
...
technique10 BuildGeometry
{
	pass P0
	{
		...
		SetGeometryShader( ConstructGSWithSO( CompileShader( gs_4_0, GS() ), "POSITION.xyz; NORMAL.xyz" ) );
		SetPixelShader( 0 );
	}
}
