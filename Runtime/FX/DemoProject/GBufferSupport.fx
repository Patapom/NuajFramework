// Holds the method to pack/unpack data to and from the G buffer
//

// This is the structure used to store in the multiple render targets for deferred rendering
...
struct PS_OUT
{
	...
	float4	Color2	: SV_TARGET2;	// VELOCITY TARGET => Velocity (WORLD SPACE) (XYZ) + ? (W)
};
...
Texture2D	GBufferTexture1		: GBUFFER_TEX1;		// Contains Normal (XY) + Depth (Z) + Specular Power / Roughnessrr (W)
Texture2D	GBufferTexture2		: GBUFFER_TEX2;		// Contains 3D Velocity in WORLD space (XYZ) + ? (W)
Texture2D	GBufferTexture3		: GBUFFER_TEX3;		// Contains Emissive (XYZ) + Extinction (W)
Texture2D	DepthBuffer			: DEPTH_BUFFER;		// The scene's final Z buffer
float3		GBufferSize			: GBUFFER_SIZE;		// X=width, Y=Height, Z=0
float3		GBufferInvSize		: GBUFFER_INV_SIZE;	// X=1/width, Y=1/Height, Z=0
...
// Reads depth from the Z buffer
//	_ScreenPosition, the screen position in [0,1] to read back pixel infos from
//
float	ReadDepth( float2 _ScreenPosition )
{
	float	Zp = DepthBuffer.SampleLevel( LinearClamp, _ScreenPosition, 0 ).x;
	float	Q = CameraData.w / (CameraData.w - CameraData.z);	// Zf / (Zf - Zn)
	return Q * CameraData.z / (Q - Zp);
}