...
// That one uses 3D noise and is not as convincing
float3	LameDisplacement( float3 _GrassPosition )
{
//	float3	WindPosition1 = 0.025 * Time * WindDirection + float3( 0.1, 0.1, 0.1 );
//	float	WindForce = NHQs( 0.5 * (_GrassPosition + WindPosition), NoiseTexture0 );
	float	WindForce = NHQs( 0.1 * _GrassPosition + 1.0 * WindPosition0, NoiseTexture0 );
			WindForce += 0.5 * NHQs( 0.2 * _GrassPosition + 2.0 * WindPosition1, NoiseTexture1 );
	return 0.05 * WindForce * WindDirection;
}