...
namespace SequencorEditor
{
	...
	public class	VerticalColorSliderControl : System.Windows.Forms.UserControl
	{
		...
		protected Vector3						m_RGB = AdobeColors.HSL_to_RGB( new AdobeColors.HSL( 1, 1, 1 ) );
		...
		public Vector3		RGB
		{
			...
			set
			{
				m_RGB = new Vector3( Math.Max( value.X, MIN_COMPONENT_VALUE ), Math.Max( value.Y, MIN_COMPONENT_VALUE ), Math.Max( value.Z, MIN_COMPONENT_VALUE ) );
...
//			base.OnPaint( e );
		...
		protected void Draw_Style_Red()
		{
			...
			for ( int i = 0; i < this.Height - 8; i++ ) //	i represents the current line of pixels we want to draw horizontally
			{
				...
				Pen		pen = new Pen( AdobeColors.ConvertHDR2LDR( new Vector3( red, m_RGB.Y, m_RGB.Z ) ) );	//	Get the Color for this line
		...
		protected void Draw_Style_Green()
		{
			...
			for ( int i = 0; i < this.Height - 8; i++ ) //	i represents the current line of pixels we want to draw horizontally
			{
				...
				Pen		pen = new Pen( AdobeColors.ConvertHDR2LDR( new Vector3( m_RGB.X, green, m_RGB.Z ) ) );	//	Get the Color for this line
		...
		protected void Draw_Style_Blue()
		{
			...
			for ( int i = 0; i < this.Height - 8; i++ ) //	i represents the current line of pixels we want to draw horizontally
			{
				...
				Pen		pen = new Pen( AdobeColors.ConvertHDR2LDR( new Vector3( m_RGB.X, m_RGB.Y, blue ) ) );	//	Get the Color for this line
		...
		protected void ResetHSLRGB()
		{
			...
			switch ( m_DrawStyle )
			{
				...
				case ColorPickerForm.DRAW_STYLE.Brightness :
					...
					break;
				case ColorPickerForm.DRAW_STYLE.Red :
					m_RGB = new Vector3( 1.0f - (float) (m_iMarker_Start_Y * InvHeight), m_RGB.Y, m_RGB.Z );
					m_HSL = AdobeColors.RGB_to_HSL( m_RGB );
					break;
				case ColorPickerForm.DRAW_STYLE.Green :
					m_RGB = new Vector3( m_RGB.X, 1.0f - (float) (m_iMarker_Start_Y * InvHeight), m_RGB.Z );
					m_HSL = AdobeColors.RGB_to_HSL( m_RGB );
					break;
				case ColorPickerForm.DRAW_STYLE.Blue :
					m_RGB = new Vector3( m_RGB.X, m_RGB.Y, 1.0f - (float) (m_iMarker_Start_Y * InvHeight) );
					m_HSL = AdobeColors.RGB_to_HSL( m_RGB );
					break;
			}
		}
